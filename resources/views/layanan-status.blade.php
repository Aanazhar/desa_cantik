    ...
    <section class="section">
        <div class="container status-page-grid">
            ...
            <div class="content-card status-check-card">
                ...
                <form method="POST" action="{{ route('layanan.status.check') }}" class="status-form">
                    @csrf
            ...
            <div class="content-card status-result-card">
                @if(isset($requestData) && $requestData)
                    ...
                    <h2 style="margin-bottom: 8px;">{{ $requestData->service->title ?? 'Pengajuan Layanan Surat' }}</h2>
                    ...
                    <div class="status-timeline">
                        <div class="timeline-step {{ in_array($st, ['pending', 'diproses', 'selesai']) ? 'completed' : '' }}">
                            <div class="step-icon">1</div>
                            <div class="step-label">Diterima</div>
                        </div>
                        <div class="timeline-step {{ in_array($st, ['diproses', 'selesai']) ? 'active' : '' }}">
                            <div class="step-icon">2</div>
                            <div class="step-label">Diproses</div>
                        </div>
                        <div class="timeline-step {{ $st === 'selesai' ? 'completed' : '' }}">
                            <div class="step-icon">3</div>
                            <div class="step-label">Selesai</div>
                        </div>
                    </div>

                    <div class="status-badge status-{{ $st }}" style="margin-bottom: 20px; font-weight: 800; font-size: 0.95rem;">
                        📌 Status Surat: {{ strtoupper($requestData->status ?? 'DITERIMA') }}
                    </div>

                    <dl class="status-details">
                        <div>
                            <dt>Kode Pengajuan</dt>
                            <dd><strong style="color: #0f766e; font-size: 1.15rem;">{{ $requestData->tracking_code }}</strong></dd>
                        </div>

                        <div>
                            <dt>Nama Pemohon</dt>
                            <dd>{{ $requestData->nama }}</dd>
                        </div>

                        <div>
                            <dt>No. WhatsApp</dt>
                            <dd>{{ $requestData->no_hp ?: '-' }}</dd>
                        </div>

                        <div>
                            <dt>Tanggal Pengajuan</dt>
                            <dd>{{ $requestData->created_at?->format('d M Y H:i') }} WIB</dd>
                        </div>
...
@endif
                    ...
                    {{-- AMBIL NOMOR WA DINAMIS DARI DATABASE & FORMAT FORMAT 628... --}}
                    @php
                    ...
                    <div style="margin-top: 25px; pt: 15px; border-top: 1px solid #f1f5f9; display: flex; justify-content: flex-end;">
                        <a href="https://api.whatsapp.com/send?phone={{ $waPhone }}&text={{ rawurlencode($waMessage) }}" target="_blank" class="btn-wa-confirm">
    ...
    {{-- MODAL POP-UP SUKSES PENGAJUAN (SEKETIKA MUNCUL DI TENGAH MATA USER) --}}
                ...
                @if($trackingCode)
                    <div class="tracking-code-box">
                        ...
                        <div class="tracking-code-display">
                            <strong id="trackingCodeText">{{ $trackingCode }}</strong>
