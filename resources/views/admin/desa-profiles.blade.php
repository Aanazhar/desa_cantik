...
<div class="profil-page">
    ...
    <header class="profil-header">
        ...
        <div class="profil-header-actions">
            <a href="{{ route('profil') }}" target="_blank" rel="noopener noreferrer" class="profil-btn profil-btn-outline">
                Lihat Website ↗
            </a>
        </div>
    </header>

    {{-- ALERT PESAN BERHASIL & ERROR --}}
    @if(session('success'))
        <div class="profil-alert profil-alert-success">✓ {{ session('success') }}</div>
    @endif
    ...
    {{-- FORM UTAMA PROFIL DESA (SECTION 01 - 06) --}}
    <form method="POST" action="{{ Route::has('admin.desa-profiles.save') ? route('admin.desa-profiles.save') : (Route::has('admin.desa-profile.store') ? route('admin.desa-profile.store') : url('/admin/desa-profiles')) }}" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="id" value="{{ $desa->id ?? '' }}">

        {{-- 01. TENTANG DESA (DENGAN INPUT NAMA DESA WAJIB) --}}
            ...
            <div class="profil-card-body">
                <div class="profil-grid" style="margin-bottom:18px;">
                    <div class="profil-field profil-field-full">
                        ...
                        <input type="text" name="name" value="{{ old('name', $desa->name ?? ($activeDesa->name ?? 'Desa Waha')) }}" required placeholder="Contoh: Desa Waha">
                    </div>
                </div>

                <div class="profil-field">
                    <label>Deskripsi Tentang Desa</label>
                    <textarea name="description" rows="7">{{ old('description', $desa->description ?? '') }}</textarea>
                </div>
            </div>
        </section>

        {{-- 02. PROFIL KEPALA DESA --}}
        <section class="profil-card">
            <div class="profil-card-header">
                <div class="profil-card-title">
            ...
            <div class="profil-card-body">
                <div class="profil-grid">
                    <div class="profil-field">
                        ...
                        <input type="text" name="head_name" value="{{ old('head_name', $desa->head_name ?? '') }}">
                    </div>
                    <div class="profil-field">
                        <label>Foto Kepala Desa</label>
                        <div class="profil-upload">
                            <div class="profil-preview">
                                @if(!empty($desa->head_photo))
                                    <img src="{{ asset(ltrim($desa->head_photo, '/')) }}" alt="Foto Kepala Desa">
                                @else 👤 @endif
                            </div>
        ...
        <section class="profil-card">
            ...
            <div class="profil-card-body">
                <div class="profil-field">
                    ...
                    <textarea name="history" rows="7">{{ old('history', $desa->history ?? '') }}</textarea>
                </div>
            </div>
        </section>
            ...
            <div class="profil-card-body">
                <div class="profil-grid">
                    <div class="profil-field">
                        ...
                        <textarea name="vision" rows="6">{{ old('vision', $desa->vision ?? '') }}</textarea>
                    </div>
                    <div class="profil-field">
                        <label>Misi Desa</label>
                        <textarea name="mission" rows="6">{{ old('mission', $desa->mission ?? '') }}</textarea>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="profil-card">
            ...
            <div class="profil-card-body">
                <div class="profil-field">
                    ...
                    <textarea name="boundaries" rows="6">{{ old('boundaries', $desa->boundaries ?? '') }}</textarea>
                </div>
                ...
                <div class="profil-field profil-field-full" style="margin-top:18px;">
                    ...
                    <div class="profil-upload">
                        <div class="profil-preview">
                            @if(!empty($desa->map_image))
                                <img src="{{ asset(ltrim($desa->map_image, '/')) }}" alt="Peta Desa">
                            @else 🗺️ @endif
                        </div>
        ...
        <section class="profil-card">
            ...
            <div class="profil-card-body">
                <div class="profil-field profil-field-full">
                    ...
                    <div class="profil-upload">
                        <div class="profil-preview">
                            @if(!empty($desa->structure_image))
                                <img src="{{ asset(ltrim($desa->structure_image, '/')) }}" alt="Struktur">
                            @else 🏢 @endif
                        </div>
    ...
    {{-- =====================================================
         07. INPUT PERANGKAT DESA (TATA PENOMORAN DESIMAL)
    ====================================================== --}}
    ...
    <section class="profil-card" style="margin-top: 35px;">
        ...
        <div class="profil-card-body">
            ...
            {{-- FORM TAMBAH --}}
            <div id="formTambahPerangkat" class="perangkat-add {{ ($errors->has('sort_order') || $errors->has('name') || $errors->has('position')) ? 'is-open' : '' }}">
                <form method="POST" action="{{ Route::has('admin.perangkat-desa.store') ? route('admin.perangkat-desa.store') : url('/admin/perangkat-desa') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="profil-grid profil-grid-3">

                        <div class="profil-field">
                            <label>Kode Hirarki / No. Urut</label>
                            <input type="text" name="sort_order" value="{{ old('sort_order') }}" required placeholder="Contoh: 1, 2, 3, 3.1, 3.2, 4">
                        ...
                        <div class="profil-field">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Nama lengkap & gelar">
                        </div>

                        <div class="profil-field">
                            <label>Jabatan</label>
                            <input type="text" name="position" value="{{ old('position') }}" required placeholder="Contoh: Kaur Keuangan / Staf">
                        </div>
            ...
            {{-- LIST KELOLA PERANGKAT --}}
            @if(isset($structures) && $structures->count())
                    ...
                    @foreach($structures as $item)
                        <article class="perangkat-card">
                            ...
                            <div class="perangkat-photo">
                                @if(!empty($item->photo))
                                    <img src="{{ asset(ltrim($item->photo, '/')) }}" alt="{{ $item->name }}">
                                @else
                                    <div class="perangkat-initial">
                                        {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($item->name, 0, 1)) }}
                                    </div>
                                @endif
                                <span class="perangkat-status">{{ $item->active ? 'Aktif' : 'Nonaktif' }}</span>
                            </div>

                            <div class="perangkat-body">
                                <div class="perangkat-order">Kode No. {{ $item->sort_order }}</div>
                                <h3>{{ $item->name }}</h3>
                                <p>{{ $item->position }}</p>
                                ...
                                <div class="perangkat-actions">
                                    ...
                                    <details class="profil-details">
                                        ...
                                        <div class="perangkat-edit">
                                            <form method="POST" action="{{ Route::has('admin.perangkat-desa.update') ? route('admin.perangkat-desa.update', $item) : url('/admin/perangkat-desa/'.$item->id) }}" enctype="multipart/form-data">
                                                @csrf
                                                @method('PUT')
                                                <div class="perangkat-edit-grid">
                                                    <div>
                                                        <label>Kode Hirarki</label>
                                                        <input type="text" name="sort_order" value="{{ old('sort_order', $item->sort_order) }}" required placeholder="Contoh: 3.1">
                                                    </div>
                                                    <div>
                                                        <label>Nama</label>
                                    ...
                                    {{-- TOGGLE --}}
                                    <form method="POST" action="{{ Route::has('admin.perangkat-desa.toggle') ? route('admin.perangkat-desa.toggle', $item) : url('/admin/perangkat-desa/'.$item->id.'/toggle') }}">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    {{-- DELETE --}}
                                    <form method="POST" action="{{ Route::has('admin.perangkat-desa.destroy') ? route('admin.perangkat-desa.destroy', $item) : url('/admin/perangkat-desa/'.$item->id) }}" onsubmit="return confirm('Hapus perangkat ini?')">
                                        @csrf
                                        @method('DELETE')
