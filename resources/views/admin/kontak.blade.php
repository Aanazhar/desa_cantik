...
@section('title', 'Kelola Kontak & Media Sosial Desa - ' . ($activeDesa->name ?? 'Desa Waha'))

{{-- HELPER RENDER IKON APPS RESMI MODERN (FLAT STYLE) --}}
@php
...
<div class="contact-admin">
    ...
    <div class="contact-admin-grid">
        ...
        <div class="contact-card-box">
            ...
            <form method="POST" action="{{ route('admin.kontak.store') }}">
                ...
                <div class="contact-field-group">
                    ...
                    {{-- KOTAK PREVIEW IKON OTOMATIS SAAT PILIH DROPDOWN --}}
                ...
                <div class="contact-field-group">
                    ...
                    <input name="title" value="{{ old('title') }}" placeholder="Contoh: WhatsApp Layanan Desa" required>
                </div>

                <div class="contact-field-group full">
                    <label id="urlLabel">URL / Nilai Kontak *</label>
                    <input name="url" id="contactUrl" value="{{ old('url') }}" placeholder="Pilih jenis kontak terlebih dahulu" required>
                ...
                <div class="contact-field-grid">
                    <!-- <div class="contact-field-group">
                        ...
                        <input name="icon" id="contactIcon" value="{{ old('icon') }}" placeholder="Otomatis terisi ikon brand">
                    </div> -->
                ...
                <div class="contact-field-group full">
                    ...
                    <textarea name="description" rows="3" placeholder="Contoh: Layanan setiap hari kerja jam 08:00 - 15:00 WITA.">{{ old('description') }}</textarea>
                </div>
        ...
        <div class="contact-card-box">
            ...
            @forelse($kontak as $contact)
                <details class="saved-contact-item" {{ $loop->first ? 'open' : '' }}>
                    <summary class="saved-contact-summary">
                        {!! renderContactBrandIcon($contact->type ?? 'other', $contact->icon) !!}
                        <div class="saved-contact-info">
                            <strong>{{ $contact->title }}</strong>
                            <small>{{ $contact->url ?: 'Belum ada isi link/nilai' }}</small>
                            <span class="saved-contact-badge">{{ ucfirst($contact->type ?? 'other') }}</span>
                        </div>
                        ...
                        <form method="POST" action="{{ route('admin.kontak.update', $contact->id) }}">
                            ...
                            <div class="contact-field-grid">
                                <div class="contact-field-group">
                                    ...
                                    <select name="type" required>
                                        @foreach([
                                            ...
                                            <option value="{{ $v }}" @selected(($contact->type ?? 'other') === $v)>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="contact-field-group">
                                    ...
                                    <input name="title" value="{{ $contact->title }}" required>
                                </div>
                            </div>
                            ...
                            <div class="contact-field-grid">
                                <!-- <div class="contact-field-group">
                                    ...
                                    <input name="icon" value="{{ $contact->icon }}">
                                </div> -->

                                <div class="contact-field-group">
                                    <label>Urutan</label>
                                    <input type="number" name="sort_order" value="{{ $contact->sort_order }}" min="0">
                                </div>
                            </div>

                            <div class="contact-field-group full">
                                <label>Keterangan</label>
                                <textarea name="description" rows="3">{{ $contact->description }}</textarea>
                            </div>

                            <div class="contact-action-bar">
                                <button class="btn-save-edit" type="submit">💾 Simpan Perubahan</button>
                                @if($contact->url)
                                    <a class="btn-open-link" href="{{ $contact->url }}" target="_blank" rel="noopener noreferrer">🔗 Tes Buka Link</a>
                                @endif
                            </div>
                        </form>

                        <form method="POST" action="{{ route('admin.kontak.destroy', $contact->id) }}" onsubmit="return confirm('Yakin ingin menghapus kontak ini?')" style="margin-top: 10px;">
                            @csrf
                            @method('DELETE')
