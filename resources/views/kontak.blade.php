...
@section('title', 'Hubungi Kami - ' . ($activeDesa->name ?? 'Desa Waha'))

{{-- HELPER SMART-DETECTION IKON APPS RESMI MODERN --}}
@php
...
<div class="contact-page">
    ...
    <div class="contact-container">
        <div class="contact-box">
            ...
            @if($displayGridContacts->count())
                <div class="contact-grid">
                    @foreach($displayGridContacts as $contact)
                        @php
                        ...
                        @if($href)
                            <a class="contact-card" href="{{ $href }}" @if($external) target="_blank" rel="noopener noreferrer" @endif>
                        @else
                            <div class="contact-card">
                        @endif

                            {{-- IKON LOGO APPS RESMI --}}
                            {!! renderContactBrandIcon($contact->type ?? 'other', $contact->icon, $contact->title) !!}

                            <div class="contact-info">
                                <strong>{{ $contact->title }}</strong>
                                <p>{{ $contact->description ?: 'Hubungi kami melalui kanal resmi ini.' }}</p>
                            </div>
                            @if($href)<div class="contact-arrow">→</div>@endif
                        @if($href)</a>@else</div>@endif
                    @endforeach
                </div>

                {{-- MEDIA SOSIAL DESA --}}
                @if($showSeparateSocialSection)
                    <div class="contact-bottom">
                        ...
                        <div class="social-grid">
                            @foreach($socialContacts as $contact)
                                ...
                                @endphp
                                <a class="social-card" href="{{ $surl }}" target="_blank" rel="noopener noreferrer">
                                    {!! renderContactBrandIcon($contact->type ?? 'other', $contact->icon, $contact->title) !!}
                                    <span class="social-name">{{ $contact->title }}</span>
                                </a>
                            @endforeach
                        </div>
                    </div>
