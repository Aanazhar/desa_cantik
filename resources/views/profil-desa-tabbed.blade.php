...
@section('title', 'Profil - ' . ($desa->name ?? ($activeDesa->name ?? 'Desa Waha')))
    ...
    <section class="pub-hero">
        <div class="public-container pub-hero-inner">
            <div>
                ...
                <h1>{{ $pageTitle ?? ('Profil ' . ($desa->name ?? ($activeDesa->name ?? 'Desa Waha'))) }}</h1>
    ...
    <div class="public-container">
        ...
        <section id="tentang" class="user-section-card">
            ...
            <div class="user-section-content">
                {{ $desa->description ?? ($activeDesa->description ?? 'Informasi tentang desa belum diisi.') }}
            </div>
        </section>
            ...
            <div class="kades-card">
                <div class="kades-photo">
                    @if(!empty($desa->head_photo))
                        <img src="{{ asset(ltrim($desa->head_photo, '/')) }}" alt="{{ $desa->head_name ?? 'Kepala Desa' }}">
                    @elseif(!empty($activeDesa->head_photo))
                        <img src="{{ asset(ltrim($activeDesa->head_photo, '/')) }}" alt="{{ $activeDesa->head_name ?? 'Kepala Desa' }}">
                    @else 
                        👤 
                    @endif
                </div>
                <div class="kades-info">
                    <span class="kades-title">Kepala Desa {{ $desa->name ?? ($activeDesa->name ?? 'Desa Waha') }}</span>
                    <h3>{{ $desa->head_name ?? ($activeDesa->head_name ?? 'Belum diatur') }}</h3>
                    <p class="user-section-content" style="margin:0;">
                        Selamat datang di portal informasi resmi Desa {{ $desa->name ?? ($activeDesa->name ?? 'Desa Waha') }}. Kami siap melayani masyarakat secara maksimal.
                    </p>
                </div>
            </div>
        </section>
        ...
        <section id="sejarah" class="user-section-card">
            ...
            <div class="user-section-content">
                {{ $desa->history ?? ($activeDesa->history ?? 'Informasi sejarah desa belum diisi.') }}
            </div>
        </section>
        ...
        <section id="visi-misi" class="user-section-card">
            ...
            <div class="visi-misi-grid">
                <div class="visi-box">
                    <h3>🎯 Visi Desa</h3>
                    <div class="user-section-content">{{ $desa->vision ?? ($activeDesa->vision ?? 'Visi desa belum diisi.') }}</div>
                </div>
                <div class="misi-box">
                    <h3>🚀 Misi Desa</h3>
                    <div class="user-section-content">{{ $desa->mission ?? ($activeDesa->mission ?? 'Misi desa belum diisi.') }}</div>
                </div>
            </div>
        </section>

        {{-- 5. PETA DESA & WILAYAH (LENGKAP DENGAN GAMBAR PETA KAPANPUN DIUPLOAD) --}}
        <section id="peta" class="user-section-card">
            <div class="user-section-header">
                <div class="user-section-icon">🗺️</div>
                <h2>Peta & Batas Wilayah</h2>
            </div>
            <div class="user-section-content">{{ $desa->boundaries ?? ($activeDesa->boundaries ?? 'Informasi batas wilayah belum diisi.') }}</div>
            ...
            @if($mapImg)
                <div class="image-preview-container">
                    <img src="{{ asset(ltrim($mapImg, '/')) }}" alt="Peta Desa {{ $desa->name ?? ($activeDesa->name ?? 'Desa') }}">
                </div>
            @endif
        </section>

        {{-- 6. STRUKTUR ORGANISASI DESA --}}
        <section id="struktur" class="user-section-card">
            ...
            @if($structImg)
                <div class="image-preview-container">
                    <img src="{{ asset(ltrim($structImg, '/')) }}" alt="Struktur Organisasi {{ $desa->name ?? ($activeDesa->name ?? 'Desa') }}">
                </div>
            @else
        ...
        {{-- 7. BAGAN STRUKTUR PERANGKAT DESA --}}
        <section id="perangkat" class="user-section-card">
            ...
            @if($majorLevels->count())

                <div class="org-chart-wrapper">
                    <div class="org-chart-tree">

                        @foreach($majorLevels as $majorNum => $levelItems)

                            <div class="org-level-row" data-level="{{ $majorNum }}">
                                ...
                                <div class="org-nodes-wrapper {{ $mainItems->count() > 1 ? 'has-multiple' : '' }}">

                                    @foreach($mainItems as $itemIndex => $mainItem)
                                        ...
                                        <div class="org-column-group">
                                            ...
                                            <div class="org-node-item">
                                                <div class="org-box">
                                                    <div class="org-photo">
                                                        @if(!empty($mainItem->photo))
                                                            <img src="{{ asset(ltrim($mainItem->photo, '/')) }}" alt="{{ $mainItem->name }}">
                                                        @else
                                                            {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($mainItem->name, 0, 1)) }}
                                                        @endif
                                                    </div>
                                                    <div class="org-details">
                                                        <span class="org-position">{{ $mainItem->position }}</span>
                                                        <span class="org-name">{{ $mainItem->name }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="org-sub-nodes">
                                                    @foreach($subItems as $subItem)
                                                        <div class="org-sub-box">
                                                            <div class="org-photo">
                                                                @if(!empty($subItem->photo))
                                                                    <img src="{{ asset(ltrim($subItem->photo, '/')) }}" alt="{{ $subItem->name }}">
                                                                @else
                                                                    {{ \Illuminate\Support\Str::upper(\Illuminate\Support\Str::substr($subItem->name, 0, 1)) }}
                                                                @endif
                                                            </div>
                                                            <div class="org-details">
                                                                <span class="org-position">{{ $subItem->position }}</span>
                                                                <span class="org-name">{{ $subItem->name }}</span>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
