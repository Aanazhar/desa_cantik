...
@section('title', 'Data & Statistik Desa - ' . ($activeDesa->name ?? 'Desa Waha'))
...
<div class="data-page">
    ...
    {{-- HERO BANNER + CONTROLS SAMPING KANAN RAPI --}}
    <section class="data-desa-hero">
        <div class="container data-desa-hero-inner">
            <div>
                ...
                <h1>
                    @if($currentCategory === 'perumahan')
                        🏠 Data Perumahan & Fasilitas Desa
                    @elseif($currentCategory === 'jaminan_sosial')
                        🤝 Data Jaminan & Kesejahteraan Sosial
                    @elseif($currentCategory === 'sosial_demografis')
                        👥 Data Sosial Demografis Kependudukan
                    @else
                        📊 DATA {{ strtoupper($activeDesa->name ?? 'DESA WAHA') }}
                    @endif
                </h1>
                <p>Data resmi statistik desa tahun pendataan {{ $selectedYear ?? date('Y') }}.</p>
            </div>

            {{-- KONTROL SAMPING KANAN PRESISI SEJAJAR --}}
            <div class="data-hero-controls">
                <div class="control-year-group">
                    <label for="yearSelect">Tahun Pendataan</label>
                    <form method="GET" action="{{ route('data-desa') }}" style="margin:0;">
                        @if(request('category'))
                ...
                <a class="btn-hero-export" href="{{ route('data-desa.export', ['format' => 'csv', 'year' => $selectedYear, 'category' => $currentCategory]) }}">
                    ↓ Export CSV
                </a>

                <a class="btn-hero-export" href="{{ route('data-desa.export', ['format' => 'pdf', 'year' => $selectedYear, 'category' => $currentCategory]) }}">
                    ↓ Export PDF
                </a>
            </div>
        </div>
    ...
    <div class="container">
        ...
        <nav class="data-nav">
            <a href="{{ route('data-desa') }}" class="{{ $currentCategory === 'ringkasan' ? 'active' : '' }}">
                📊 Ringkasan Data
            </a>
            <a href="{{ route('data-desa.category', 'perumahan') }}" class="{{ $currentCategory === 'perumahan' ? 'active' : '' }}">
                🏠 Perumahan
            </a>
            <a href="{{ route('data-desa.category', 'jaminan_sosial') }}" class="{{ $currentCategory === 'jaminan_sosial' ? 'active' : '' }}">
                🤝 Jaminan Sosial
            </a>
            <a href="{{ route('data-desa.category', 'sosial_demografis') }}" class="{{ $currentCategory === 'sosial_demografis' ? 'active' : '' }}">
                👥 Sosial Demografis
            </a>
        </nav>
        ...
        <div class="data-summary-grid">
            <div class="summary-item">
                ...
                <strong>{{ number_format((int)($displayStats['total_penduduk'] ?? 0), 0, ',', '.') }}</strong>
                <small>Jiwa</small>
            </div>
            <div class="summary-item">
                <span>🏠 Kepala Keluarga</span>
                <strong>{{ number_format((int)($displayStats['kepala_keluarga'] ?? 0), 0, ',', '.') }}</strong>
                <small>KK</small>
            </div>
            <div class="summary-item">
                <span>👨 Laki-laki</span>
                <strong>{{ number_format((int)($displayStats['laki_laki'] ?? 0), 0, ',', '.') }}</strong>
                <small>Jiwa</small>
            </div>
            <div class="summary-item">
                <span>👩 Perempuan</span>
                <strong>{{ number_format((int)($displayStats['perempuan'] ?? 0), 0, ',', '.') }}</strong>
                <small>Jiwa</small>
            </div>
        </div>
...
<script>
    ...
    if (document.getElementById('villageChart')) {
        DesaCharts.bar(document.getElementById('villageChart'), @json($villages->pluck('nama_dusun')->values()), @json($villages->pluck('jumlah_penduduk')->map(fn($v) => (int)$v)->values()), 'jiwa');
    }
